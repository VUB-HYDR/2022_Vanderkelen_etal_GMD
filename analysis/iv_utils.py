...
def lunit2grid(var,infile,outfile,select_lunit=0):
    from datetime import datetime

    # Function to convert lunit to grid for one file and variable
    ...
    # load grid data
    lunit_ixy = ds.land1d_ixy.values #  2d longitude index of corresponding lunit
    lunit_jxy = ds.land1d_jxy.values #  2d latitude index of corresponding lunit
    pft_ixy   = ds.pfts1d_ixy.values #  2d longitude index of corresponding pft
    pft_jxy   = ds.pfts1d_jxy.values #  2d latitude index of corresponding pft
    lat_mod   = ds.lat        #  2d latitude grid
    lon_mod   = ds.lon        # 2d longitude grid

    # load vegetation type
    pft1d_lunit_ctl = ds.pfts1d_itype_lunit.values
    ...
    # load variable 
    var_vect = ds[var].values

    
    # -------
    # 2. Manipulations

    print('Manipulating '+var+' data')

    # get grid info
    nlat = len(lat_mod)
    nlon = len(lon_mod)
    nlunit = int(max(ds.land1d_ityplunit.values)) # number of lunits
    nt = len(date) # number of output timesteps

    # get number of points to loop over
    nlunitvector = len(pft_ixy)

    # preprare for loop
    var_lunit = np.empty((nt,nlat,nlon,nlunit+1))

    # convert from lunit to lat/lon

    for j in range(nt-1):
        for k in range(nlunitvector-1):

            # get lunit and grid indices
            lunit = int(pft1d_lunit_ctl[k])
            loni  = int(pft_ixy[k])-1
            lati  = int(pft_jxy[k])-1

            # assign values to grid
            var_lunit[j,lati,loni,lunit] = var_vect[j,k]

    # -------
    # 3. Save data again

    # new variable name
    varname_new = var+'_lunit'

    da_new = xr.DataArray(data=var_lunit, dims=["time","lat","lon","lunit"], coords=[ ("time",time), ("lat", lat_mod), ("lon", lon_mod), ("lunit", range(nlunit+1))], attrs = ds[var].attrs)
    
    if select_lunit>0: 
        da_new = da_new.sel(lunit=select_lunit)

    ds_new = da_new.to_dataset(name=varname_new)
    ds_new.assign_attrs(ds.attrs)
    if outfile != None: 
        ds_new.to_netcdf(outfile)     
    

    # print timing
    print("Time spend: " +str(datetime.now()-start))
